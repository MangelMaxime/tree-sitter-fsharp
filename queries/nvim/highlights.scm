...
; @type rule so Helix's last-capture-in-source-order makes @variable.member
...
((long_identifier . (identifier) @_root (identifier) @variable.member)
 (#lua-match? @_root "^[a-z_]"))
...
; @type/@module/@attribute, but tree-sitter's last-in-source-order
; resolution means the inner @variable.member here wins for the
; trailing identifier unless a more-specific override re-captures it.
; The explicit overrides further down re-apply @type / @module /
; @attribute to the trailing identifier in those contexts.
(long_identifier . (identifier) (identifier) @variable.member .)
...
(namespace_decl
  name: (long_identifier) @module)

(module_decl
  name: (long_identifier) @module)

; Override: in type/namespace/attribute contexts, the previous rules tinted
; individual identifiers as @variable.member or @type (PascalCase
; heuristic). Re-capture EACH identifier as the more-specific @type /
; @module / @attribute so the whole long_identifier matches its
...
((type_application_expression
   (type_expression (long_identifier (identifier) @_wildcard)))
  (#eq? @_wildcard "_"))
((type_application_expression
   (type_expression (long_identifier) @_wildcard))
  (#eq? @_wildcard "_"))
...
(namespace_decl name: (long_identifier (identifier) @module))
(module_decl name: (long_identifier (identifier) @module))
(module_decl abbrev: (long_identifier (identifier) @module))
...
(module_decl abbrev: (long_identifier) @module)

(import_decl
  [
    (long_identifier) @module
  ])
...
((member_self_ident (identifier) @_wildcard)
  (#lua-match? @_wildcard "^_+$"))
((member_self_ident) @_wildcard
  (#lua-match? @_wildcard "^_+$"))
...
(record_field
  name: (long_identifier) @variable.member)
...
; resolution the @variable.member tint wins for record field names
; instead of falling through to @type via the wrapping long_identifier.
(record_field
  name: (long_identifier (identifier) @variable.member))
...
(dot_expression member: (identifier) @variable.member)

; Dynamic-access operator `obj?member` — `?` as operator, member as a member ref.
(dynamic_expression "?" @operator)
(dynamic_expression member: (identifier) @variable.member)
...
; Uses @variable.member (Helix's property scope; the theme has no `property`).
(computation_expression
  args: (parenthesized_expression
          (binary_expression left: (long_identifier (identifier) @variable.member))))
(computation_expression
  args: (parenthesized_expression
          (tuple_expression
            (binary_expression left: (long_identifier (identifier) @variable.member)))))
...
(computation_expression
  method: (long_identifier (identifier) @variable.member))
...
; PascalCase / @variable.member heuristics earlier are overridden.
((nameof_expression
   (long_identifier (identifier) @type))
 (#lua-match? @type "^[A-Z]"))
...
((application_expression
 ...
 (#any-of? @function.builtin "raise" "reraise")
 (#lua-match? @type "^[A-Z]"))

; Exception-raising functions — highlighted like throw/raise in other languages
((long_identifier . (identifier) @function.builtin .)
 (#any-of? @function.builtin "raise" "reraise" "failwith" "failwithf" "invalidArg" "invalidOp" "nullArg"))
...
((application_expression
   . (long_identifier . (identifier) @function.builtin .))
 (#any-of? @function.builtin "byte" "sbyte" "int8" "uint8" "int16" "uint16" "int32" "uint32" "int64" "uint64" "int" "uint" "nativeint" "unativeint" "float" "float32" "double" "single" "decimal" "char" "string" "bigint" "enum"))

; base and fixed are reserved keywords but appear as plain identifiers in the tree
((identifier) @keyword (#any-of? @keyword "base" "fixed"))
