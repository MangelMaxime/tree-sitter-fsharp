...
; `not`/`nameof`/etc., so they share the @keyword slot rather
; than the generic @keyword used for declaration/control keywords.
["not" "upcast" "downcast" "nameof" "lazy" "assert"] @keyword
...
(preproc_elif_kw) @preproc
(preproc_else_kw) @preproc
(line_directive) @preproc
(preproc_endif_kw) @preproc
...
; @type rule so Helix's last-capture-in-source-order makes @property
...
; @type/@type/@attribute, but tree-sitter's last-in-source-order
; resolution means the inner @property here wins for the
; trailing identifier unless a more-specific override re-captures it.
; The explicit overrides further down re-apply @type / @type /
; @attribute to the trailing identifier in those contexts.
(long_identifier . (identifier) (identifier) @property .)
...
(measure_expression (type_parameter) @type)
...
(namespace_decl
  name: (long_identifier) @type)

(module_decl
  name: (long_identifier) @type)

; Override: in type/namespace/attribute contexts, the previous rules tinted
; individual identifiers as @property or @type (PascalCase
; heuristic). Re-capture EACH identifier as the more-specific @type /
; @type / @attribute so the whole long_identifier matches its
...
(namespace_decl name: (long_identifier (identifier) @type))
(module_decl name: (long_identifier (identifier) @type))
(module_decl abbrev: (long_identifier (identifier) @type))
(import_decl (long_identifier (identifier) @type))
...
(module_decl abbrev: (long_identifier) @type)
...
; `this` / `self` / `_` self identifier on instance members. @variable.special
...
(member_self_ident) @variable.special

; Underscore-only member-self identifiers (`member _.Foo = …`,
; `member __.Foo = …`, etc.) are placeholders — strip the
; @variable.special styling so they render with the editor's default
...
; resolution the @property tint wins for record field names
; instead of falling through to @type via the wrapping long_identifier.
(record_field
  name: (long_identifier (identifier) @property))
...
(dynamic_expression member: (identifier) @property)
...
((long_identifier . (identifier) @type . (identifier) @function .)
 (#match? @type "^[A-Z]") (#match? @function "^[a-z_]"))
((long_identifier . (identifier) @type . (identifier) @type . (identifier) @function .)
 (#match? @type "^[A-Z]") (#match? @function "^[a-z_]"))
((long_identifier . (identifier) @type . (identifier) @type . (identifier) @type . (identifier) @function .)
 (#match? @type "^[A-Z]") (#match? @function "^[a-z_]"))
...
((application_expression . (long_identifier . (identifier) @type . (identifier) @function .))
 (#match? @type "^[A-Z]"))
((application_expression . (long_identifier . (identifier) @type . (identifier) @type . (identifier) @function .))
 (#match? @type "^[A-Z]"))
((application_expression . (long_identifier . (identifier) @type . (identifier) @type . (identifier) @type . (identifier) @function .))
 (#match? @type "^[A-Z]"))
((application_expression . (long_identifier . (identifier) @type . (identifier) @type . (identifier) @type . (identifier) @type . (identifier) @function .))
 (#match? @type "^[A-Z]"))
...
; Uses @property (Helix's property scope; the theme has no `property`).
(computation_expression
  args: (parenthesized_expression
          (binary_expression left: (long_identifier (identifier) @property))))
(computation_expression
  args: (parenthesized_expression
          (tuple_expression
            (binary_expression left: (long_identifier (identifier) @property)))))
...
(computation_expression
  method: (long_identifier (identifier) @property))
...
; `@keyword` because they direct query progression — same
; semantic role as `for`/`match`/`if` above.
(query_operator op: _ @keyword)
(query_join_operator
  ["join" "in" "on"] @keyword)
(query_group_by_operator
  ["groupBy" "groupValBy" "groupJoin" "into"] @keyword)
(query_left_outer_join_operator
  ["leftOuterJoin" "in" "on" "into"] @keyword)
...
; PascalCase / @property heuristics earlier are overridden.
((nameof_expression
   (long_identifier (identifier) @type))
 (#match? @type "^[A-Z]"))
