...
; `not`/`nameof`/etc.; Zed has no keyword.operator scope, so plain @keyword.
...
; or-pattern `|`, so it reads as a keyword rather than @operator.
(and_pattern "&" @keyword)
...
; @type rule so last-capture-in-source-order makes @property win for these
; chains. Capitalised-rooted paths (`System.Threading.Foo`) don't match here
; (the root `#match?` fails), so they keep their @type segments.
...
; @type/@namespace/@attribute, but last-in-source-order resolution means
; the inner @property here wins for the trailing identifier unless a
; more-specific override re-captures it. The explicit overrides further
; down re-apply @type / @namespace / @attribute to the trailing
; identifier in those contexts.
...
(namespace_decl
  name: (long_identifier) @type @namespace)

(module_decl
  name: (long_identifier) @type @namespace)

; Override: in type/namespace/attribute contexts, the previous rules tinted
; individual identifiers as @property or @type (PascalCase heuristic).
; Re-capture EACH identifier as the more-specific @type / @namespace /
; @attribute so the whole long_identifier matches its containing context,
; not the expression-context heuristics.
; (Highlight resolution picks the LAST capture in source order.)
...
; later match (resolution: last capture in source order) and the
; wildcard renders with the editor's default text color. Limited to
; this construct — `_` in other type positions (e.g. `typedefof<_ list>`)
; keeps the existing @type styling.
...
(namespace_decl name: (long_identifier (identifier) @type @namespace))
(module_decl name: (long_identifier (identifier) @type @namespace))
(module_decl abbrev: (long_identifier (identifier) @type @namespace))
(import_decl (long_identifier (identifier) @type @namespace))
...
(module_decl abbrev: (long_identifier) @type @namespace)

(import_decl
  [
    (long_identifier) @type @namespace
  ])
...
; than the broad @type captures above, so on last-capture resolution the
; @property tint wins for record field names instead of falling through
; to @type via the wrapping long_identifier.
(record_field
...
; @keyword because they direct query progression — same semantic role
; as `for`/`match`/`if` above.
...
; `raise` long_identifier ends up with @function.builtin colour — the
; last matching capture wins, and capturing `raise` here with a
; non-themed name would otherwise wipe out the builtin tint.
...
; @function.builtin isn't a documented Zed scope, but Zed resolves unknown
; dotted captures by longest known prefix, so it degrades to @function; themes
; that do style function.builtin get the richer colour.
((long_identifier . (identifier) @function.builtin .)
